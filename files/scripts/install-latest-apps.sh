...
install_latest_appimage() {
  ...
  url="$(gh_asset_url "$repo" "$pattern")"
  path="/opt/${binary}/${binary}.AppImage"
  icon_dir="/usr/share/icons/hicolor/256x256/apps"
  echo "Installing latest ${desktop_name} AppImage from ${url}"
  install -d -m 0755 "/opt/${binary}" "$icon_dir" /usr/share/applications
  retry curl -fL --retry 4 --retry-delay 10 -o "$path" "$url"
  chmod 0755 "$path"
  cat >"/usr/local/bin/${binary}" <<EOFAPP
#!/usr/bin/env bash
exec "${path}" "\$@"
EOFAPP
...
install -d -m 0755 /opt/lmstudio /usr/share/applications /usr/local/bin
retry curl -fL --retry 4 --retry-delay 10 -o /opt/lmstudio/LM_Studio.AppImage 'https://lmstudio.ai/download/latest/linux/x64?format=AppImage'
chmod 0755 /opt/lmstudio/LM_Studio.AppImage
cat >/usr/local/bin/lmstudio <<'EOFLMS'
#!/usr/bin/env bash
exec /opt/lmstudio/LM_Studio.AppImage "$@"
EOFLMS
chmod 0755 /usr/local/bin/lmstudio
cat >/usr/share/applications/lmstudio.desktop <<'EOFDESKTOP'
[Desktop Entry]
Name=LM Studio
Comment=Local LLM management and inference
Exec=/usr/local/bin/lmstudio %U
Terminal=false
Type=Application
...
retry bash -c 'curl -fsSL https://opencode.ai/install | OPENCODE_INSTALL_DIR=/usr/local/bin bash'

# werman RNNoise plugin release: install LADSPA plugin and keep the full bundle under /usr/local/lib/rnnoise.
install -d -m 0755 /usr/local/lib/rnnoise /usr/local/lib/ladspa
...
cp -a /tmp/linux-rnnoise/. /usr/local/lib/rnnoise/
...
install -Dm755 "${ladspa_so}" /usr/local/lib/ladspa/librnnoise_ladspa.so
