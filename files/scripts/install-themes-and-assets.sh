...
gh_asset_url_optional() {
  ...
  assets_json="$(curl -fsSL "${headers[@]}" "$api_url" | jq -c '.assets // []')"
...
install_darkly() {
  local fedora_release url rpm_file src_dir
  fedora_release="$(rpm -E %fedora)"
  url="$(gh_asset_url_optional "Bali10050/Darkly" "darkly-.*\\.fc${fedora_release}\\.x86_64\\.rpm$")"

  if [[ -n "$url" ]]; then
    rpm_file="/tmp/darkly-${fedora_release}.rpm"
    echo "Installing Darkly from latest Fedora ${fedora_release} RPM: ${url}"
    retry curl -fL --retry 4 --retry-delay 10 -o "$rpm_file" "$url"
    retry "${DNF[@]}" install --skip-unavailable "$rpm_file"
    rm -f "$rpm_file"
    return 0
  fi

  echo "WARN: no Fedora ${fedora_release} Darkly RPM release asset found, building Darkly from source" >&2
  src_dir="/tmp/Darkly"
...
install_macos_tahoe_theme_bundle() {
  ...
  # Upstream ships some cursor aliases as text files. Convert them to symlinks so
  # KDE's cursor KCM and applications detect the themes reliably.
  for cursor_theme in \
  ...
  gtk-update-icon-cache -q /usr/share/icons/DDCmacOsTahoe-icons-dark || true
  rm -rf "$src_dir"
}
...
smoke_check_theme_assets() {
  ...
  for path in "${required_paths[@]}"; do
    if [[ ! -e "$path" ]]; then
      echo "ERROR: expected theme asset missing: $path" >&2
      exit 1
    fi
  done
}
...
"${DNF[@]}" clean all || true
rm -rf /var/cache/dnf /var/cache/libdnf5 /tmp/Darkly /tmp/darkly-gtk /tmp/Beauty-Plasma-Themes /tmp/macOsTahoeKdeTheme /tmp/WhiteSur-kde /tmp/McMojave-kde /tmp/WhiteSur-cursors || true
