...
- recipe.yml is not valid YAML or is missing required keys
...
- scripts missing the required lib.sh source directive
"""
...
for wf in sorted((ROOT / ".github" / "workflows").glob("*.yml")):
    for lineno, line in enumerate(wf.read_text().splitlines(), start=1):
        ...
        if ref in FLOATING_REFS:
            fail(f"{wf.name}:{lineno}: floating ref @{ref} in `uses: {action}`")
            policy_failed = True
        else:
...
# All scripts (except lib.sh itself) must source lib.sh early.
lib_sh = ROOT / "files" / "scripts" / "lib.sh"
if lib_sh.is_file():
    ok("shared library files/scripts/lib.sh exists")
    for script in sorted((ROOT / "files" / "scripts").glob("*.sh")):
        if script.name == "lib.sh":
            continue
        content = script.read_text()
        if "source" in content and "lib.sh" in content:
            ok(f"{script.name} sources lib.sh")
        else:
            fail(f"{script.name} does not source lib.sh")
else:
    fail("shared library files/scripts/lib.sh is missing")

# --- script shellcheck compliance --------------------------------------------
# Basic check: scripts should have set -euo pipefail
for script in sorted((ROOT / "files" / "scripts").glob("*.sh")):
    content = script.read_text()
    if "set -euo pipefail" in content:
        ok(f"{script.name} has set -euo pipefail")
    else:
        fail(f"{script.name} is missing set -euo pipefail")
